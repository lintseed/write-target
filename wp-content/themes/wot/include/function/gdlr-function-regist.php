...
<?php			
		...
		function gdlr_get_default_admin_option() {
			...
			if(empty($sidebar_option)){
				update_option('gdlr_sidebar_name', 
					array( 'blog', 'blog-left', 'portfolio', 'portfolio-left', 'shortcodes',  'woocommerce',  'Features', 'Archives', 'homepage-blog', 'contact')
				);
			}
	...
	if( !function_exists('gdlr_excerpt_more') ){
		function gdlr_excerpt_more( $more ) {
			...
			return '... <div class="clear"></div><a href="' . get_permalink() . '" class="gdlr-button without-border excerpt-read-more">' . __( 'Read More', 'gdlr_translate' ) . '</a>';
		}
	}
