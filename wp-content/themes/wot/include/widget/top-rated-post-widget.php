<?php
/**
 * Plugin Name: Goodlayers Top Rated Post
...
add_action( 'widgets_init', 'gdlr_top_rated_post_widget' );
if( !function_exists('gdlr_top_rated_post_widget') ){
	function gdlr_top_rated_post_widget() {
		register_widget( 'Goodlayers_Top_Rated_Post' );
	}
}

if( !class_exists('Goodlayers_Top_Rated_Post') ){
	class Goodlayers_Top_Rated_Post extends WP_Widget{

		// Initialize the widget
		function __construct() {
			parent::WP_Widget(
				'gdlr-top-rated-post-widget', 
				__('Goodlayers Top Rated Post Widget','gdlr_translate'), 
				array('description' => __('A widget that show top rated posts', 'gdlr_translate')));  
		}
		...
		function widget( $args, $instance ) {
			...
			$query_args['meta_key'] = 'gdlr-post-rating';
			...
			if($query->have_posts()){
				echo '<div class="gdlr-top-rated-post-widget">';
				while($query->have_posts()){ $query->the_post();
					$gdlr_post_option = get_post_meta(get_the_ID(), 'post-option', true);
					$gdlr_post_option = json_decode($gdlr_post_option, true);
				
					echo '<div class="top-rated-post-widget">';
					echo '<div class="top-rated-post-rating">' . $gdlr_post_option['rating'] . '</div>';
					echo '<div class="top-rated-post-title"><a href="' . get_permalink() . '" >' . get_the_title() . '</a></div>';
