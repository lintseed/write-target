	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			if($atts['type'] == 'none' || (empty($atts['title']) && empty($atts['caption']))){
				if( !empty($atts['additional_html']) || !empty($atts['carousel']) ){
					$ret  = '<div class="gdlr-item-title-wrapper pos-none ' . $atts['additional_class'] . '">';
				...
				if($atts['type'] == 'left'){ $title_class = ' pos-icon gdlr-left '; }
				else{ $title_class = ' pos-' . $atts['type']; }
			
				$ret  = '<div class="gdlr-item-title-wrapper ' . $atts['additional_class'] . $title_class . '">';
				if(!empty($atts['title'])){
					$ret .= '<h3 class="gdlr-item-title gdlr-skin-title gdlr-skin-border">';
					$ret .= !empty($atts['carousel'])? '<i class="icon-angle-left gdlr-flex-prev"></i>': '';
					if( ($atts['type'] == 'icon'|| $atts['type'] == 'left') && !empty($atts['icon']) ){
						$ret .= '<span class="gdlr-item-title-icon"><i class="' . $atts['icon'] . '"></i></span>';
					}
					$ret .= '<span class="gdlr-item-title-gimmick gdlr-left"></span>';
					$ret .= '<span>' . $atts['title'] . '</span>';
					$ret .= '<span class="gdlr-item-title-gimmick gdlr-right"></span>';
					$ret .= !empty($atts['carousel'])? '<i class="icon-angle-right gdlr-flex-next"></i>': '';
					$ret .= '</h3>';
				}
				if( $atts['type'] != 'icon' && $atts['type'] != 'left' && !empty($atts['caption']) ){
	...
	if( !function_exists('gdlr_get_feature_media_item') ){
		function gdlr_get_feature_media_item( $settings ){
			...
			if( !empty($settings['title']) ){
				$ret .= '<h3 class="feature-media-title" >';
				if( !empty($settings['title-icon']) ){
					$ret .= '<span class="feature-media-icon">';
					$ret .= '<i class="' . $settings['title-icon'] . '" ></i>';
					$ret .= '</span>';
				}			
				$ret .= gdlr_text_filter($settings['title']);
				$ret .= '</h3>';
				$ret .= '<div class="feature-media-divider"></div>';
			}
	...
	if( !function_exists('gdlr_get_skill_round_item') ){
		function gdlr_get_skill_round_item( $settings ){	
			...
			$ret .= '<img src="' . GDLR_PATH . '/images/skill-round.png" alt="" />';
			$ret .= '<div class="gdlr-round-skill-overlay" style="';
			$ret .=	'background-color: ' . $settings['background-color'] . ';';
			$ret .=	'opacity: ' . $settings['background-opacity'] . ';';
			$ret .=	'filter: alpha(opacity=' . (floatval($settings['background-opacity'])*100) . ');';
			$ret .= '" ></div>';
